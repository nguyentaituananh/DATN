...
class FavoriteController {
	...
	addFavorite = asyncHandler(async (req, res) => {
		...
		const newFavorite = await FavoriteService.addFavorite(user_id, req.body)
	...
	getFavoritesByUser = asyncHandler(async (req, res) => {
		const user_id = req.user._id // Lấy user_id từ authentication
		const { limit = 50, skip = 0 } = req.query
	...
	// Xóa sản phẩm yêu thích theo product_id
	deleteFavoriteByProduct = asyncHandler(async (req, res) => {
		const user_id = req.user._id // Lấy user_id từ authentication
		const { productId } = req.params
		const result = await FavoriteService.deleteFavoriteByUserAndProduct(user_id, productId)
